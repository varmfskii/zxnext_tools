...
{
  for (uint32_t i = 0; i < cfg->bulk_size; i += cfg->block_size)
    tsf_init_chunk(cfg, i + cfg->bulk_start);

  return TSF_RES_OK;
}

TSF_RESULT tsf_init_chunk(TSF_CONFIG *cfg, uint32_t addr)
{
  ...
  uint32_t magic = TSF_MAGIC;
  cfg->hal_write_func(addr, &magic, sizeof(magic));

  return TSF_RES_OK;
}

TSF_RESULT tsf_check_blank(TSF_CONFIG *cfg, uint32_t addr, uint32_t size)
{
  while (size)
  {
    uint16_t sz = (uint16_t)min(cfg->buf_size, size);
    uint8_t *buf = cfg->buf;
...
TSF_RESULT tsf_take_new_chunk(TSF_VOLUME *vol, uint8_t type, uint32_t *chunk_addr)
{
  if (!vol->free) return TSF_RES_BULK_FULL;   // to speed up the search
  
  TSF_CONFIG *cfg = vol->cfg;

  uint16_t chunk_num = cfg->last_written_chunk;
  uint32_t addr = chunk_addr(chunk_num);
  TSF_CHUNK chunk;
...
TSF_RESULT tsf_search(TSF_VOLUME *vol, uint32_t *chunk_addr, const char *name)
{
  TSF_CONFIG *cfg = vol->cfg;
  uint8_t fnlen = (uint8_t)strlen(name);

  for (uint32_t i = 0; i < cfg->bulk_size; i += cfg->block_size)
  {
    uint32_t addr = i + cfg->bulk_start;
    TSF_CHUNK chunk;
    cfg->hal_read_func(addr, &chunk, sizeof(TSF_CHUNK));
...
TSF_RESULT tsf_open(TSF_VOLUME *vol, TSF_FILE *file, const char *name, uint8_t mode)
{
  file->mode = mode;
  file->vol = vol;
...
TSF_RESULT tsf_open_for_read(TSF_FILE *file, const char *name)
{
  ...
  file->seek = 0;
  file->chunk_offset = sizeof(TSF_CHUNK) + sizeof(TSF_HDR) + (uint8_t)strlen(name);
...
TSF_RESULT tsf_read(TSF_FILE *file, void *buf, uint32_t size)
{
  return tsf_read_int(file, buf, size);
}

TSF_RESULT tsf_seek(TSF_FILE *file, uint32_t size)
{
  return tsf_read_int(file, 0, size);
}

TSF_RESULT tsf_read_int(TSF_FILE *file, void *buf, uint32_t size)
{
  uint8_t *data = (uint8_t*)buf;
  TSF_CONFIG *cfg = file->vol->cfg;
  uint32_t addr = file->chunk_addr;
  ...
  while (size)
  {
    ...
    uint16_t sz = (uint16_t)min(cfg->block_size - file->chunk_offset, size);
...
TSF_RESULT tsf_write(TSF_FILE *file, const void *buf, uint32_t size)
{
  uint8_t *data = (uint8_t*)buf;
  TSF_CONFIG *cfg = file->vol->cfg;
  uint32_t addr = file->chunk_addr;
...
TSF_RESULT tsf_list(TSF_VOLUME *vol, uint8_t flag)
{
  ...
  for (uint32_t addr = last_addr; addr < (cfg->bulk_start + cfg->bulk_size); addr += cfg->block_size)
  {
    TSF_CHUNK chunk;
    cfg->hal_read_func(addr, &chunk, sizeof(TSF_CHUNK));
